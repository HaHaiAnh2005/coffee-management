...
export const POSPage: React.FC = () => {
  ...
  // Chỉ Thu ngân, Quản lý hoặc Chủ quán mới có quyền đóng ca trực tiếp
  ...
  const [overrideModal, setOverrideModal] = useState<{
    ...
    title: string;
    details: string;
    type: 'ITEM_CANCEL' | 'BILL_CANCEL' | 'DISCOUNT_OVERRIDE' | 'SHIFT_CLOSE_OVERRIDE';
    targetId: string;
    onSuccess: () => void;
  }>({
    isOpen: false,
            ...
            <button
              onClick={() => {
                if (canCloseShift) {
                  ...
                  setOverrideModal({
                    isOpen: true,
                    title: 'Yêu Cầu Quản Lý Duyệt Đóng Ca',
                    details: `Tài khoản hiện tại (${user?.name || 'Nhân viên'} - ${user?.role || 'N/A'}) không có quyền đóng ca POS. Chỉ có Thu Ngân hoặc Quản Lý mới được phép. Vui lòng nhập mã PIN Quản Lý (1234) hoặc Chủ Quán (9999) để xác thực đóng ca!`,
                    type: 'SHIFT_CLOSE_OVERRIDE',
                    targetId: currentShift?.id || 'POS-SHIFT',
                    onSuccess: () => {
                      setIsCloseShiftModal(true);
                    },
                  });
              ...
              title={
                canCloseShift
                  ? 'Kết toán & Đóng ca làm việc'
                  : 'Chỉ Thu ngân hoặc Quản lý mới được đóng ca (Cần PIN duyệt)'
              }
            >
