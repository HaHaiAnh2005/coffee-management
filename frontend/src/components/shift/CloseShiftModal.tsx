...
export const CloseShiftModal: React.FC<CloseShiftModalProps> = ({
  ...
  // Chỉ Thu ngân, Quản lý hoặc Chủ quán mới có quyền đóng ca POS
  const canCloseShift = Boolean(user?.role && ['CASHIER', 'MANAGER', 'ADMIN'].includes(user.role));
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!canCloseShift) {
      alert('Quyền hạn bị từ chối: Chỉ Thu ngân hoặc Quản lý cửa hàng mới có quyền Kết toán & Đóng ca POS!');
      return;
    }
      ...
      <form onSubmit={handleSubmit} className="p-2 space-y-4">
        ...
        {!canCloseShift && (
          <div className="p-3 rounded-xl bg-amber-50 border border-amber-200 text-amber-900 text-xs font-semibold flex items-center gap-2">
            <FiAlertCircle className="w-4 h-4 text-amber-700 shrink-0" />
            <span>
              Quyền hạn bị giới hạn: Chỉ có vai trò <strong>Thu ngân</strong> hoặc <strong>Quản lý / Chủ quán</strong> mới được phép Đóng ca POS.
            </span>
          </div>
        )}
        ...
        <div className="pt-2 border-t border-stone-200 flex justify-end gap-2">
          ...
          <button
            ...
            className={`px-5 py-2.5 rounded-xl text-xs font-bold shadow-md flex items-center gap-1.5 transition-all ${
              canCloseShift
                ? 'bg-gradient-to-r from-stone-800 to-stone-900 hover:from-stone-900 hover:to-black text-white cursor-pointer'
                : 'bg-stone-300 text-stone-500 cursor-not-allowed opacity-60'
            }`}
          >
            <FiLock /> {canCloseShift ? 'Kết toán & Đóng ca' : 'Không có quyền đóng ca'}
          </button>
        </div>
      </form>
    </Modal>
